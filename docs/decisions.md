...
2026-07-03 | DEV_SKIP_AUTH в .env.local обходит вход в dev-режиме (getCurrentUser отдаёт фиксированного dev-юзера, proxy.ts пропускает всё) | пользователь попросил не логиниться каждый раз при разработке. Гейт: только NODE_ENV !== "production" — не может утечь в прод. Выключить: DEV_SKIP_AUTH=false в .env.local
